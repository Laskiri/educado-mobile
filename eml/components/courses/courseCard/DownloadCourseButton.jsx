...
import { Image, View, Text, Modal, TouchableOpacity, TouchableWithoutFeedback } from 'react-native';
        ...
        <TouchableWithoutFeedback>
          ...
          <View className="w-[90%] bg-projectLightGray rounded-xl p-5">
              <View className="flex-row justify-between items-center">
                ...
                <MaterialCommunityIcons size={24} name="close" color={'gray'} onPress={() =>  setModalVisible(false)}></MaterialCommunityIcons>
              </View>
              <Text className="text-base text-projectBlack mb-2.5">
                Você tem certeza que deseja excluir o download do curso? Você ainda pode
                assisti-lo com acesso à internet e baixá-lo novamente.
              </Text>
              <View className="flex-row justify-between w-100 items-center">
                <TouchableOpacity onPress={() => setModalVisible(false)}>
                  <Text className="text-primary_custom text-xl border-b border-primary_custom font-bold">Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={isDownloaded ? handleRemove : handleDownload}
                  className="bg-error rounded-lg py-4 px-10"
                >
                  <Text className="font-bold text-projectWhite text-xl">{isDownloaded ? 'Excluir' : 'Baixar'}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </TouchableWithoutFeedback>
